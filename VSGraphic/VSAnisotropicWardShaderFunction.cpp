...
IMPLEMENT_RTTI(VSAnisotropicWardShaderFunction,VSShaderMainFunction)
BEGIN_ADD_PROPERTY(VSAnisotropicWardShaderFunction,VSShaderMainFunction)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(VSAnisotropicWardShaderFunction)
IMPLEMENT_INITIAL_END
VSAnisotropicWardShaderFunction::VSAnisotropicWardShaderFunction(const VSUsedName &ShowName,VSMaterial * pMaterial)
...
bool VSAnisotropicWardShaderFunction::GetFuntionString(VSString &OutString)const
{
	...
	else
	{
		...
		OutString += VSRenderer::ms_pRenderer->Float3() +  _T("WorldCameraDir")  + _T(" = ");
		...
		for (unsigned int i = 0 ; i < VSLight::LT_MAX ; i++)
		{
			...
			{			
				...
				VSRenderer::ms_pRenderer->AnisotropicWardDirectionalLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSX]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSY]->GetNodeName().GetString(),VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
			...
			else if(i == VSLight::LT_POINT)
			{
				...
				VSRenderer::ms_pRenderer->AnisotropicWardPointLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSX]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSY]->GetNodeName().GetString(),VSShaderStringFactory::ms_WorldPos,VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);

			}
			else if(i == VSLight::LT_SPOT)
			{

				VSRenderer::ms_pRenderer->AnisotropicWardSpotLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSX]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESSY]->GetNodeName().GetString(),VSShaderStringFactory::ms_WorldPos,VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
...
VSAnisotropicWardShaderFunction::~VSAnisotropicWardShaderFunction()
{
