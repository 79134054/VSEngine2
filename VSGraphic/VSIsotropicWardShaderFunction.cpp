...
IMPLEMENT_RTTI(VSIsotropicWardShaderFunction,VSShaderMainFunction)
BEGIN_ADD_PROPERTY(VSIsotropicWardShaderFunction,VSShaderMainFunction)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(VSIsotropicWardShaderFunction)
IMPLEMENT_INITIAL_END
VSIsotropicWardShaderFunction::VSIsotropicWardShaderFunction(const VSUsedName &ShowName,VSMaterial * pMaterial)
...
bool VSIsotropicWardShaderFunction::GetFuntionString(VSString &OutString)const
{
	...
	else
	{
		...
		for (unsigned int i = 0 ; i < VSLight::LT_MAX ; i++)
		{
			...
			{			
				...
				VSRenderer::ms_pRenderer->IsotropicWardDirectionalLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESS]->GetNodeName().GetString(),VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),UseLookUpTable(),ShadowStringArray[i],OutString);
			...
			else if(i == VSLight::LT_POINT)
			{
				...
				VSRenderer::ms_pRenderer->IsotropicWardPointLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESS]->GetNodeName().GetString(),VSShaderStringFactory::ms_WorldPos,VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),UseLookUpTable(),ShadowStringArray[i],OutString);

			}
			else if(i == VSLight::LT_SPOT)
			{

				VSRenderer::ms_pRenderer->IsotropicWardSpotLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString()
					,m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),m_pInput[IN_ROUGHNESS]->GetNodeName().GetString(),VSShaderStringFactory::ms_WorldPos,VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),UseLookUpTable(),ShadowStringArray[i],OutString);
...
void VSIsotropicWardShaderFunction::SetGlobleValue(unsigned int uiOutPutStringType , VSVShader * pVShader , VSPShader * pPShader)
{
	if (uiOutPutStringType == OST_MATERIAL)
	{
		if (pPShader && UseLookUpTable())
		{
			VSTexAllState * pTex = (VSTexAllState *)VSTexAllState::GetIsotropicWardLookUpTable();
			static VSUsedName PSIsotropicWardLookUpTableSampler = VSShaderStringFactory::ms_PSIsotropicWardLookUpTableSampler;
			pPShader->SetParam(PSIsotropicWardLookUpTableSampler,pTex);
		}
	}
}
VSIsotropicWardShaderFunction::~VSIsotropicWardShaderFunction()
{
