...
IMPLEMENT_RTTI(VSShirleyShaderFunction,VSShaderMainFunction)
BEGIN_ADD_PROPERTY(VSShirleyShaderFunction,VSShaderMainFunction)
END_ADD_PROPERTY
IMPLEMENT_INITIAL_BEGIN(VSShirleyShaderFunction)
IMPLEMENT_INITIAL_END
VSShirleyShaderFunction::VSShirleyShaderFunction(const VSUsedName &ShowName,VSMaterial * pMaterial)
...
bool VSShirleyShaderFunction::GetFuntionString(VSString &OutString)const
{
	...
	else
	{
		...
		for (unsigned int i = 0 ; i < VSLight::LT_MAX ; i++)
		{
			...
			{			
				...
				VSRenderer::ms_pRenderer->ShirleyDirectionalLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_NU]->GetNodeName().GetString(),m_pInput[IN_NV]->GetNodeName().GetString(),m_pInput[IN_RS]->GetNodeName().GetString(),
					VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
			...
			else if(i == VSLight::LT_POINT)
			{
				...
				VSRenderer::ms_pRenderer->ShirleyPointLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_NU]->GetNodeName().GetString(),m_pInput[IN_NV]->GetNodeName().GetString(),m_pInput[IN_RS]->GetNodeName().GetString(),
					VSShaderStringFactory::ms_WorldPos,VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);

			}
			else if(i == VSLight::LT_SPOT)
			{

				VSRenderer::ms_pRenderer->ShirleySpotLight(iLightNum[i],m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(),
					m_pInput[IN_NU]->GetNodeName().GetString(),m_pInput[IN_NV]->GetNodeName().GetString(),m_pInput[IN_RS]->GetNodeName().GetString(),
					VSShaderStringFactory::ms_WorldPos,VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"),ShadowStringArray[i],OutString);
...
VSShirleyShaderFunction::~VSShirleyShaderFunction()
{
