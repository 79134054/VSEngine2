...
bool VS1DTexSampler::GetFuntionString(VSString &OutString)const
{
	if(VSRenderer::ms_pRenderer)
		OutString += VSRenderer::ms_pRenderer->Tex1D(this);

	if (m_uiVESRGB)
	{
		VSString NodeString = VSRenderer::ms_pRenderer->GetValueElement(GetOutputNode(VS1DTexSampler::OUT_COLOR),m_uiVESRGB);
	...
	if (m_uiVEDecode)
	{
		VSString NodeString = VSRenderer::ms_pRenderer->GetValueElement(GetOutputNode(VS1DTexSampler::OUT_COLOR),m_uiVEDecode);
...
void VS1DTexSampler::SetTexture(VSTexAllStateR * pTexAllState)
{
	if(!pTexAllState)
		return;
	if(pTexAllState->GetResource()->m_pTex->GetTexType() != VSTexture::TT_1D)
		return ;
	m_pTexAllState = pTexAllState;
}
bool VS1DTexSampler::GetOutPutValueString(VSString &OutString)const
{
	VSString Temp;
