...
protected:
	...
	VSStaticMeshNode * DebugCLODMesh;
	VSREAL m_fPencent;
};
...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	VSWorld::ms_pWorld->CreateActor(_T("CLODMonster.STMODEL"), VSVector3(0, 0, 0), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
	...
	VSStaticMeshNodeR * MeshNodeR = (VSStaticMeshNodeR *)VSResourceManager::LoadResource(_T("CLODMonster.STMODEL"), false);
	if (MeshNodeR)
	{
		DebugCLODMesh = MeshNodeR->GetResource();
	}
	return true;
}
...
bool VSDemoWindowsApplication::PostUpdate()
{
	...
	if (DebugCLODMesh)
	{
		VSGeometryNode * pGeometryNode = DebugCLODMesh->GetGeometryNode(0);
		for (unsigned int i = 0; i < pGeometryNode->GetNormalGeometryNum(); i++)
		{
			VSGeometry * pGeometry = pGeometryNode->GetNormalGeometry(i);
			pGeometry->GetMeshDate()->SetLodDesirePercent(m_fPencent);
			pGeometry->GetMeshDate()->UpDateClodMesh();
		}
	}
	return true;
}
...
bool VSDemoWindowsApplication::OnDraw()
{
	...
	VSRenderer::ms_pRenderer->DrawText(0, 640, VSDWCOLORRGBA(255, 255, 255, 255), _T("Use Key '+' And '-' to Change CLodParamter"));
	VSRenderer::ms_pRenderer->DrawText(0, 660, VSDWCOLORRGBA(255, 255,255, 255), _T("CLodParamter:%f"), m_fPencent);
	return true;
}
