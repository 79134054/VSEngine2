...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	VSSkelectonActor * pSKActor3 = (VSSkelectonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithSocket.SKMODEL"), VSVector3(0, 0, 800), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
	pSKActor3->GetTypeNode()->PlayAnim(_T("Attack"), 1.0f, VSController::RT_WRAP);

	VSStaticActor * pSTActor1 = (VSStaticActor *)VSWorld::ms_pWorld->CreateActor<VSStaticActor>(VSVector3(0, 0, 0), VSMatrix3X3::ms_Identity, VSVector3(0.4f,0.4f,0.4f), m_pTestMap);

	pSKActor3->AddChildActor(pSTActor1);
	pSTActor1->GetTypeNode()->AttachParentSocket(_T("HandSocket"));
