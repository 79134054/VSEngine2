//加载Demo14.14的模型，读者可以跟踪下混合流程，用“+”“-”按键调节阈值查看混合的效果。
...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	pSKActor3 = (VSSkelectonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithTwoImmediateAnim.SKMODEL"), VSVector3(0, 0, 300), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
...
bool VSDemoWindowsApplication::PostUpdate()
{
	...
	static VSUsedName AnimNodeName = _T("OneParamBlend");
	pSKActor3->GetTypeNode()->SetAnimTreeNodePara(AnimNodeName, (void *)&m_fPencent);
...
bool VSDemoWindowsApplication::OnDraw()
{
	...
	VSRenderer::ms_pRenderer->DrawText(0, 640, VSDWCOLORABGR(255,255,255,255), _T("Use Key '+' And '-' to Change ImmediateAnimBlendParamter"));
	VSRenderer::ms_pRenderer->DrawText(0, 660, VSDWCOLORABGR(255, 255, 255, 255), _T("ImmediateAnimBlendParamter:%f"), m_fPencent);
	return true;
}
