...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	VSMaterialRPtr  pLightFun = VSResourceManager::LoadASYNMaterial(_T("LightFunction.MATERIAL"), false);
	...
	pPointLightActor->GetTypeNode()->SetShadowType(VSPointLight::ST_DUAL_PARABOLOID);
	pPointLightActor->GetTypeNode()->SetLightMaterial(pLightFun);
	pPointLightActor->GetTypeNode()->m_Diffuse = VSColorRGBA(15.0f, 15.0f, 15.0f, 1.0f);
