...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	{
		for (int z = -RENDER_NUM; z <= RENDER_NUM; z++)
		{
			for (int x = -RENDER_NUM; x <= RENDER_NUM; x++)
			{
				VSWorld::ms_pWorld->CreateActor(_T("NewStone.STMODEL"),VSVector3(x * 400.0f,y * 400.0f,z * 400.0f),VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
			}
		}
