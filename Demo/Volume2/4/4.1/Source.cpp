//播放Demo14.22带MorphTree的模型。读者可以通过“+”和“-”来调节混合参数，来播放Morph脸部表情动画
...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	m_pCameraActor->GetTypeNode()->AddController(m_p1stCameraController);
 	VSVector3 CameraPos(0.0f, 450.0f, -200.0f);
	...
	pSTActor = (VSStaticActor *)VSWorld::ms_pWorld->CreateActor(_T("NewFaceMorphWithTwoMorph.STMODEL"), VSVector3(0.0f, 0.0f, 0.0f), VSMatrix3X3::ms_Identity, VSVector3(3.0f, 3.0f, 3.0f), m_pTestMap);
...
bool VSDemoWindowsApplication::PostUpdate()
{
	...
	static VSUsedName AnimNodeName = _T("OneParamSmoothMorphBlend");
	pSTActor->GetTypeNode()->SetMorphTreeNodePara(AnimNodeName, (void *)&m_fPencent);
	return true;
}
...
bool VSDemoWindowsApplication::OnDraw()
{
	...
	VSRenderer::ms_pRenderer->DrawText(0, 640, VSDWCOLORABGR(255,255,255,255), _T("Use Key '+' And '-' to Change SmoothMorphBlendParamter"));
	VSRenderer::ms_pRenderer->DrawText(0, 660, VSDWCOLORABGR(255, 255, 255, 255), _T("SmoothMorphBlendParamter:%f"), m_fPencent);
	return true;
}
