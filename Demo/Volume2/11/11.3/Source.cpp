...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	VSSkelectonActor * pSKActor1 = (VSSkelectonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithAnim.SKMODEL"), VSVector3(-600, 0, 600), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
	pSKActor1->GetTypeNode()->PlayAnim(_T("Idle"), 1.0f, VSController::RT_WRAP);

	VSSkelectonActor * pSKActor2 = (VSSkelectonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithAnim.SKMODEL"), VSVector3(0, 0, 600), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
	pSKActor2->GetTypeNode()->PlayAnim(_T("Walk"), 1.0f, VSController::RT_WRAP);

	VSSkelectonActor * pSKActor3 = (VSSkelectonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithAnim.SKMODEL"), VSVector3(600, 0, 600), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
	pSKActor3->GetTypeNode()->PlayAnim(_T("Attack"), 1.0f, VSController::RT_WRAP);

	VSSkelectonActor * pSKActor4 = (VSSkelectonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithAnim.SKMODEL"), VSVector3(0, 0, 900), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
	pSKActor4->GetTypeNode()->PlayAnim(_T("RootMotion"), 1.0f, VSController::RT_WRAP);

	VSSkelectonActor * pSKActor5 = (VSSkelectonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithAnim.SKMODEL"), VSVector3(-600, 0, 300), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
	...
	VSSkelectonActor * pSKActor7 = (VSSkelectonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithAnim.SKMODEL"), VSVector3(600, 0, 300), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
	...
	VSPostEffectSetR * pGray = VSResourceManager::LoadASYNPostEffect(_T("PostEffect_MaterialAndGray.POSTEFFECT"), false);
	VSWorld::ms_pWorld->AttachWindowViewFamilyToCamera(m_pCameraActor, VSWindowViewFamily::VT_WINDOW_NORMAL,
		_T("WindowUse"), SceneMap, VSForwordEffectSceneRenderMethod::ms_Type.GetName().GetBuffer(), -1, pGray);
...
bool VSDemoWindowsApplication::OnDraw()
{
	...
	VSRenderer::ms_pRenderer->DrawText(0, 640, VSDWCOLORABGR(255, 255, 255, 255), _T("Use Key 'P' to Switch Gray PostEffect"));
	VSRenderer::ms_pRenderer->DrawText(0, 660, VSDWCOLORABGR(255, 255, 255, 255), _T("Use Key 'M' to Switch Material PostEffect"));
	return true;
}
...
void VSDemoWindowsApplication::OnKeyDown(unsigned int uiKey)
{
	...
	else if (uiKey == VSEngineInput::BK_M)
	{

		VSViewFamily * pViewFamily = VSSceneManager::ms_pSceneManager->GetViewFamily(_T("WindowUse"));
		VSSceneRenderMethod * pRM = pViewFamily->m_pSceneRenderMethod;
		VSPostEffectSet * pPESet = pRM->GetPostEffectSet();
		static VSUsedName PEMaterialName = _T("Material");
		VSPostEffectFunction * PEMaterial = pPESet->GetPEFunctionFromShowName(PEMaterialName);
		PEMaterial->m_bEnable = !PEMaterial->m_bEnable;
	}
